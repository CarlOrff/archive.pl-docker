...
my $ua = 'Mozilla/5.0 (Linux; Android 6.0.1; Nexus 5X Build/MMB29P) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.7204.183 Mobile Safari/537.36 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)';
...
# fetch options
my %opts;
getopts('ac:Cd:Df:Fhi:k:ln:o:p:P:rsSt:T:u:vwW:x:y:Y:z:', \%opts);
...
foreach my $url ( @urls ) {
    ...
    if ($opts{F}) {
        ...
        if (!$success) {
            ...
            $content = encode('UTF-8', $r->decoded_content, Encode::FB_CROAK);
        }
...
##################################################################################################
        ...
        if ($mime =~ /(ht|x)ml/i) {
            ...
            if ($opts{a}) {
                $outfile->add_entry(
                    author => ib_encode($author),
                    link => $encoded_url,
                    summary => ib_encode($description),
                    title => ib_encode($title),
                );
            }
            else {
                $outfile .= '<li' . (length($language) > 1 ? ' lang="'.$language.'"' : '') . '>' . (length($author) > 1 ? ib_encode($author).' ' : '') . '<a ' . (length($language) > 1 ? 'hreflang="'.$language.'" ' : '') . 'href="' . $encoded_url . '">' . HTML_format_title($title) . '</a>' . HTML_format_description($description) . "</li>\n";
            }
        }
...
##################################################################################################
            ...
            if ($opts{a}) {
                $outfile->add_entry(
                    ...
                    link => $encoded_url,
                    summary => ib_encode($description),
                    title => ib_encode($title . ' [PDF]'),
                );
            }
            else {
                $outfile .= '<li>PDF: ' . (length $author > 0 ? ib_encode($author).' ' : '') . '<a href="' . $encoded_url . '" type="application/pdf">' . HTML_format_title($title) . '</a> ' . HTML_format_description($description) . "</li>\n";
            }
        }
...
sub init_blacklist {
    ...
    %blacklist = (
        ...
        'WordPress 2' => {
                'host'  => '',
                'path'  => '',
                'query' => qr/(\A|[;&])share=(facebook|email|instagram|jetpack-whatsapp|linkedin|mastodon|nextdoor|pinterest|pocket|print|reddit|telegram|tumblr|twitter|x)\b/,
        },
        'WordPress 3' => {
                'host'  => 'widgets.wp.com',
